import matplotlib
matplotlib.use('QtAgg')  # Switch to QtAgg backend for better handling
...
class Environment:
    ...
    FlagVertices = np.array([[0.25, 0.2], [0.25, 0.8], [0.75, 0.65], [0.25, 0.5]])

    def __init__(self, n=DimensionOfGround, mine_density=MineDensity, visual=False, end_game_on_mine_hit=True):
        self.n = n
        ...
        self.mine_ground_copy = np.full((self.n, self.n), None, dtype=object)
        ...
        for row in range(self.n):
            for column in range(self.n):
                ...
                self.mine_ground_copy[row, column] = self.variable_mine_ground_copy[row, column]
    ...
    def add_mine_flag(self, row, column):
        """Add a flag to the specified cell to indicate a mine."""
        if self.opened[row, column]:
            return  # Cannot flag an already opened cell
        
        if self.flags[row, column] is None:
    ...
    def undo_move(self, row, column):
        """Undo the effects of a move."""
        if 0 <= row < self.n and 0 <= column < self.n:
            if self.flags[row, column]:
                self.flags[row, column] = None
            else:
                self.opened[row, column] = False
                self.clicked[row, column] = False
                self.clicked_and_not_revealed[row, column] = False

            if self.mine_revealed[row, column]:
                self.mine_revealed[row, column] = False
                self.mine_hit = False

            if self.visual:
                self.squares_copy[row, column].set_facecolor(self.CoveredColor)
                for txt in self.ax_copy.texts:
                    if txt.get_position() == (row + 0.5, column + 0.5):
                        txt.remove()
                self.render_env()
        else:
            print(f"Tried to undo an out-of-bounds move at ({row}, {column})")
