class Variable:
    def __init__(self, value=None, row=None, column=None, constraint_value=None, has_mine=1):
        self.value = value
        self.row = row
        self.column = column
    ...
    def get_unopened_neighbours(self, env, use_probability_agent=False):
        ...
        for i in [-1, 0, 1]:
            for j in [-1, 0, 1]:
                if i == 0 and j == 0:
                    continue
                if 0 <= self.row + i < env.n and 0 <= self.column + j < env.n:
                    # Continue only if the index is within the grid
